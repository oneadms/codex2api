...
func (h *Handler) buildAccountResponse(
	...
	// 该账号实际生效的设备码（绑定值优先，老账号按行身份派生）。
	traeCNDeviceIdentity := auth.TraeCNEffectiveDeviceIdentity(row)
	...
	resp := accountResponse{
		...
		ATOnly:                 !isOpenAIResponsesAccount && !isGrokAccount && !isAntigravityAccount && !isTraeCNAccount && !isClaudeAccount && row.GetCredential("refresh_token") == "" && row.GetCredential("access_token") != "",
		...
		TraeCNMachineID:              traeCNDeviceIdentity.MachineID,
		TraeCNDeviceBoundAt:          row.GetCredential(auth.TraeCNDeviceBoundAtCredentialKey),
