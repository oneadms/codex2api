...
type accountResponse struct {
	...
	TraeCNCreditsState     string                      `json:"traecn_credits_state,omitempty"`
	...
	TraeCNDeviceBoundAt           string                     `json:"traecn_device_bound_at,omitempty"`
...
func (h *Handler) listAccountsLite(c *gin.Context, ctx context.Context) {
	...
	for _, row := range rows {
		...
		accounts = append(accounts, accountLiteResponse{
			...
			ATOnly:             !isOpenAIResponsesAccount && !isGrokAccount && !isTraeCNAccount && !isClaudeAccount && row.GetCredential("refresh_token") == "" && row.GetCredential("access_token") != "",
...
// Claude 账号仅接受 claude-* 原生模型；Trae CN 账号由专用同步接口管理；
// 空数组 = 清空白名单，放行全部模型；
...
func (h *Handler) ListRecycleBinAccounts(c *gin.Context) {
	...
	for _, row := range rows {
		...
		resp := recycleBinAccountResponse{
			...
			Email:              email,
			PlanType:           planType,
			ATOnly:             !isOpenAIResponsesAccount && !isTraeCNAccount && !isClaudeAccount && row.GetCredential("refresh_token") == "" && row.GetCredential("access_token") != "",
