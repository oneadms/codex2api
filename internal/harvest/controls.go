...
var ErrSettingNotFound = errors.New("采票设置不存在")
...
func NewCodexHarvestService(nodes CodexHarvestNodeRepository, settings SettingRepository, initial *CodexHarvestSpeed) *CodexHarvestService {
...
func (s *CodexHarvestService) Wake() <-chan struct{} { return s.wake }

func (s *CodexHarvestService) Snapshot(ctx context.Context) CodexHarvestControlSnapshot {
	...
	if err != nil {
		out.SettingsError = "采票设置读取失败，继续使用上次有效设置"
	}
	return out
}

func (s *CodexHarvestService) Rank(nodes []mihomo.HarvestNode, records []CodexHarvestNodeRecord, tried map[string]bool) []mihomo.HarvestNode {
	return rankCodexHarvestNodes(nodes, records, tried, s.explore.Add(1)-1, time.Now())
}
