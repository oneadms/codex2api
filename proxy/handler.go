...
func (h *Handler) applyUpstreamChannelFilter(c *gin.Context, effectiveModel string, filter auth.AccountFilter) auth.AccountFilter {
	...
	case database.UpstreamChannelCodex:
		return func(account *auth.Account) bool {
			if account == nil || account.IsGrokAPI() || account.IsAntigravityAPI() || account.IsTraeCNAPI() || account.IsClaudeOAuth() || account.IsClaudeAPIKey() {
				return false
			}
			return filter == nil || filter(account)
		}
...
func accountFilterForCompactResponsesModelWithOriginal(originalModel string, effectiveModel string, allowCodexAccounts bool) auth.AccountFilter {
	...
	return func(account *auth.Account) bool {
		// The dedicated compact executor has no Grok adapter, and TraeCN has no
		// Responses compact endpoint either; both need the inline path.
		return account != nil && !account.IsGrokAPI() && !account.IsTraeCNAPI() && inner(account)
	}
}
...
func accountFilterForInlineCompactionModelWithOriginal(originalModel string, effectiveModel string, allowCodexAccounts bool) auth.AccountFilter {
	...
	return func(account *auth.Account) bool {
		// Antigravity Google bearer 不能落入官方 Codex executor；TraeCN/Claude 系
		// 账号也不能走 inline compaction（TraeCN 无 compact 端点，Claude 走原生路由）。
		if account == nil || account.IsAntigravityAPI() || account.IsClaudeOAuth() || account.IsClaudeAPIKey() || account.IsTraeCNAPI() || !inner(account) {
			return false
		}
...
func (h *Handler) Responses(c *gin.Context) {
	...
	if upstreamChannel == database.UpstreamChannelTraeCN {
		requestModel = strings.TrimSpace(gjson.GetBytes(rawBody, "model").String())
		mappedModel = requestModel
	} else if upstreamChannel == database.UpstreamChannelAntigravity || nativeAntigravityModel {
...
func (h *Handler) ResponsesCompact(c *gin.Context) {
	...
	for attempt := 0; ; attempt++ {
		...
		if compactViaResponses {
			...
			resp, reqErr = executeHTTPWithContinuousRetryKeepalive(c.Request.Context(), func() (*http.Response, error) {
				return ExecuteRequest(requestCtx, account, appendCompactionTriggerToResponsesBody(codexBody), upstreamSessionID, proxyURL, apiKey, deviceCfg, downstreamHeaders, false)
			})
		} else {
			resp, reqErr = executeHTTPWithContinuousRetryKeepalive(c.Request.Context(), func() (*http.Response, error) {
				return ExecuteCompactRequest(requestCtx, account, codexBody, upstreamSessionID, proxyURL, apiKey, deviceCfg, downstreamHeaders)
			})
		}
		durationMs := int(time.Since(start).Milliseconds())
